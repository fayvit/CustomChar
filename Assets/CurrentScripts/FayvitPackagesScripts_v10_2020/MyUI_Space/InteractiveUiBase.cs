...
namespace FayvitUI_10_2020
{
    public abstract class InteractiveUiBase
    {
        ...
        {
            UiSupportSingleton.Instance.StartCoroutine(ScrollPos());
        }
        ...
        {
            contadorDeTempo = 0;
            UiSupportSingleton.Instance.StartCoroutine(MoveScroll(umaS, rowCellCount));
        ...
        public void ChangeOptionWithVal(int quanto, int rowCellCount = -1)
        {
            if (quanto != 0)
            {
                ...
                UiEventAgregator.Publish(
                    new FayvitUiEvent(UIEventKey.UiDeOpcoesChange, menuScrollRect.transform.parent.gameObject, SelectedOption));
            }
        }



        public void SelectAnOption(int qual)
        {
        ...
        protected virtual IEnumerator MoveScroll_H(AnOption[] umaS, int rowCellCount)
        {
            ...
            if (menuScrollRect.horizontalScrollbar.value != destiny)
                UiSupportSingleton.Instance.StartCoroutine(MoveScroll_H(umaS, rowCellCount));
