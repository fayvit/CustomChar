...
namespace FayvitMove_10_2020
{
    ...
    public class BasicMove
    {
        ...
        public bool IsGrounded
        {
            get
            {
                ...
                if (!isGrounded && wasGrounded)
                {
                    wasGrounded = false;
                    retornoDonoChao = true;
                    MoveSupportSingleton.Instance.StartCoroutine(GhostJumpSupport());
        ...
        public void MoveApplicator(Vector3 dir, bool run = false, bool startJump = false , bool pressJump = false)
        {
            ...
            {
                FayvitMoveEventAgregator.Publish(new FayvitMoveEvent(FayvitMoveEventKey.animateFall,transform.gameObject));
                _JumpM.StartFall();
                //AplicaGravidade();
            }
        }
        ...
        void UpdateMove(Vector3 targetDirection, bool run = false)
        {
            ...
            if (targetDirection != Vector3.zero)
            {
                ...
                if (Controller.velocity.magnitude < movFeatures.walkSpeed /*&& VerificaChao.noChao(elementos.controle, elementos.transform)*/)
                {
            ...
            FayvitMoveEventAgregator.Publish(
                new FayvitMoveEvent(
                    FayvitMoveEventKey.changeMoveSpeed, 
