...
namespace FayvitUI
{
    public abstract class InteractiveUiBase
    {
        ...
        private float CalcScrollDestiny(int numChilds,int rowCellCount)
        {
            int val = (rowCellCount == -1) ? numChilds : Mathf.CeilToInt((float)numChilds / rowCellCount);
            int opc = SelectedOption / ((rowCellCount == -1) ? 1 : rowCellCount);


            return Mathf.Clamp((float)(val - opc - 1) / Mathf.Max(val - 1, 1), 0, 1);
        }
